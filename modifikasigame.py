...
class Game(tk.Frame):
    def __init__(self, master):
        ...
        self.width = 610
        self.height = 400
        self.canvas = tk.Canvas(self, bg='#2C3E50', # Mengubah background warna
                                width=self.width,
                                height=self.height,)
        self.canvas.pack()
        self.pack()
    ...
    def draw_text(self, x, y, text, size='40'):
        font = ('Arial', size, 'bold') # mengubah font dan menebalkan font
        return self.canvas.create_text(x, y, text=text, font=font, fill='#ECF0F1') #Mengubah warna text

    def update_lives_text(self):
        text = '❤: %s' % self.lives # Mengubah icon text
        if self.hud is None:
